<div>
    <div class="card">
        ...
        <div class="card-body">
            ...
            @foreach ($selectedCoAdvisors as $index => $coAdvisorId)
                <div class="fields-co-advisor mt-2">
                    <div class="input-fields">
                        <select id="coAdvisorSelect" wire:model="selectedCoAdvisors.{{ $index }}"
                    ...
                    <button wire:click="removeCoAdvisor({{ $index }})"
            ...
            @if (count($selectedCoAdvisors) + count($externalCoAdvisors) < 3)
                <div class="input-fields mt-2">
                    <button wire:click="addCoAdvisor" class="btn btn-orange">เพิ่มที่ปรึกษาร่วม</button>
                    <button wire:click="addExternalCoAdvisor"
                        class="btn btn-orange">เพิ่มที่ปรึกษาร่วมจากภายนอก</button>
                </div>
            @endif
        </div>
    </div>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                mainAdvisorSelect.addEventListener('change', function() {
                    ...
                    let filteredCoAdvisors = coAdvisors.filter(coAdvisor => coAdvisor.id_teacher !==
                        selectedMainAdvisorId);
